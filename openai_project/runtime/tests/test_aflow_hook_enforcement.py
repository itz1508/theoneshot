...
from audisor.aflow_lifecycle.contract import accept_for_primary, frozen_readiness_decision, frozen_tree_digest
from audisor.aflow_lifecycle.hook import evaluate_hook_payload
...
def test_01_hook_configuration_uses_documented_wrapper() -> None:
    root = Path(__file__).resolve().parents[3]
    config = json.loads((root / ".codex" / "hooks.json").read_text(encoding="utf-8"))
    hook = config["hooks"]["PreToolUse"][0]["hooks"][0]
    assert "command" in hook and "commandWindows" in hook
...
def test_11_hook_reuses_existing_verifiers() -> None:
    ...
    import audisor.aflow_lifecycle.contract as contract
    import audisor.aflow_lifecycle.hook as hook
