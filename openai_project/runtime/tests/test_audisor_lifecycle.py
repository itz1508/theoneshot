...
def analysis(*, state: str = "no_material_gap", ready: bool = True, gaps: list[object] | None = None) -> dict:
    return {
        "success_definition": {"functional_outcomes": ["automatic Audisor lifecycle"]},
...
def test_qualifying_tasks_require_aflow(task_kind: str) -> None:
    assert requires_audisor_analysis(task_kind)


def test_read_only_factual_task_does_not_require_lifecycle() -> None:
    assert not requires_audisor_analysis("factual_question")
...
def test_agent_definition_is_read_only() -> None:
    agent = Path(__file__).resolve().parents[1] / "src" / "audisor" / "audisor_lifecycle" / "build_analysis.py"
...
def test_frozen_aflow_tree_is_byte_stable() -> None:
    ...
    assert frozen_tree_digest(frozen) == "3e1a025bbcdd0e808f033a28a07bf68041efba49758e95146442556b87835b8f"
