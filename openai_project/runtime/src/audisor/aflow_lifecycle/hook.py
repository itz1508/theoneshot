"""Auditable A-Flow PreToolUse control for Codex-interceptable mutations."""
...
from .contract import AflowLifecycleError, canonical_text, verify_lock
...
def _load_active_state(state_root: Path) -> Mapping[str, Any] | None:
    ...
    if not isinstance(value, Mapping):
        raise AflowLifecycleError("active lock state is malformed")
    return value
...
def _targets_authorized(targets: list[str], contract: Mapping[str, Any]) -> tuple[bool, str]:
    ...
    for target in targets:
        if not any(_inside(target, root) for root in allowed):
            return False, f"target outside allowed authority: {target}"
        if any(_inside(target, root) or _inside(root, target) for root in prohibited):
            return False, f"target conflicts with prohibited authority: {target}"
        if not any(_inside(target, path) for path in planned):
...
def evaluate_hook_payload(payload: Mapping[str, Any], state_root: Path) -> dict[str, Any]:
    ...
    try:
        if mutation:
            ...
            if not targets:
                ...
                decision, reason, exit_code = "deny", "no active A-Flow execution lock exists", 1
            else:
    ...
    record = {"event": event, "timestamp": datetime.now(UTC).isoformat(), "hook_name": "aflow_pretool", "mutation_tool": tool, "requested_targets": targets, "lock_present": lock_present, "lock_valid": lock_valid, "authority_valid": authority_valid, "decision": decision, "reason": reason, "exit_code": exit_code}
...
def main() -> int:
    ...
    state = Path(os.environ.get("AFLOW_STATE_ROOT", default_state_root()))
