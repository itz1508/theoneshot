...
The frozen ``openai_project/aflow`` package supplies the A-Flow decision.  This
...
from .contract import AflowLifecycleError, FROZEN_TO_CONTRACT_READINESS, canonical_text, normalize_frozen_readiness
...
def _error(message: str) -> None:
    raise AflowLifecycleError(message)
...
def verify_contract(contract: Mapping[str, Any]) -> bool:
    ...
    except (AflowLifecycleError, KeyError, TypeError, ValueError, json.JSONDecodeError):
        return False
...
def assemble_contract(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(frozen, Mapping) or frozen.get("decision") not in FROZEN_TO_CONTRACT_READINESS:
        _error("unknown frozen A-Flow decision")
